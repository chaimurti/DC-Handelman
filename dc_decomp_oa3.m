...
P.deg = 3;                                              %Degree of polynomial to be decomposed
offset_deg = 1;                                         %Degree offset
c1 = -5;
% c2 = 10;

gamma.deg = P.deg+offset_deg;                           %Degree of gamma(x)
nu.deg = P.deg+offset_deg;                              %Degree of nu(x)
P.monoms = lex_exps(n,P.deg+offset_deg);                           %Monomials of P(x)
 P.coeffs = zeros(1,Coeff_total(n,P.deg+offset_deg));   
%P.coeffs = 7*(rand(1,Coeff_total(n,P.deg))-0.5); %Set up a vector of coefficients for P(x)
P.coeff_total = Coeff_total(n,P.deg+offset_deg);
temp_lex = lex_index_nh([3 0]);
 P.coeffs(temp_lex) = -.027;
 temp_lex = lex_index_nh([2 0]);
 P.coeffs(temp_lex) = -.27;
